...
export function useSourceControlPanel(
  ...
  const generateCommitMessageHint = "AI commit message generation has been removed";
  ...
  const generateCommitMessage = useCallback(async () => {
    if (!repo || stagedEntries.length === 0) return;
    setActionError("AI commit message generation has been removed");
  }, [repo, stagedEntries.length]);
