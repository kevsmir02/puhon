...
# zsh, so we shadow $? into `_puhon_ret`.

{
  _puhon_user_zdotdir="${PUHON_USER_ZDOTDIR:-$HOME}"
  [ -f "$_puhon_user_zdotdir/.zshrc" ] && source "$_puhon_user_zdotdir/.zshrc"
...
if [[ -z "$__PUHON_HOOKS_LOADED" ]]; then
  __PUHON_HOOKS_LOADED=1
  ...
  _puhon_precmd() {
    local _puhon_ret=$?
    printf '\e]133;D;%s\e\\' "$_puhon_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_puhon_urlencode "$PWD")"
  ...
  _puhon_preexec() {
    ...
    [[ -n "$PUHON_BLOCKS" ]] && _puhon_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _puhon_precmd
    add-zsh-hook preexec _puhon_preexec
  fi
